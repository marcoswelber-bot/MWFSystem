  ...
  if (!clinicScope.isAdmMaster && !clinicScope.clinicId) {
    ...
    try {
      ...
      const [
        ...
        selectedPatientId ? readSupabaseList<Appointment>("appointments", appointmentsQuery.eq("patient_id", selectedPatientId).order("appointment_date", { ascending: false })) : Promise.resolve({ data: [] as Appointment[], error: undefined }),
        selectedPatientId ? readSupabaseList<FinancialTransaction>("financial_transactions", transactionsQuery.eq("patient_id", selectedPatientId).order("due_date", { ascending: false })) : Promise.resolve({ data: [] as FinancialTransaction[], error: undefined }),
        selectedPatientId ? readSupabaseList<PatientPackage>("patient_packages", packagesQuery.eq("patient_id", selectedPatientId).order("created_at", { ascending: false })) : Promise.resolve({ data: [] as PatientPackage[], error: undefined }),
        selectedPatientId ? readSupabaseList<MedicalRecord>("medical_records", recordsQuery.eq("patient_id", selectedPatientId).order("created_at", { ascending: false })) : Promise.resolve({ data: [] as MedicalRecord[], error: undefined }),
