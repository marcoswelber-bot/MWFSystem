create table if not exists public.patient_packages (
  ...
  constraint patient_packages_sessions_check check (
  ...
  constraint patient_packages_total_value_check check (total_value >= 0),
  constraint patient_packages_payment_method_check check (
    payment_method in ('pix', 'dinheiro', 'cartao', 'boleto', 'parcelado')
  ),
  constraint patient_packages_status_check check (
