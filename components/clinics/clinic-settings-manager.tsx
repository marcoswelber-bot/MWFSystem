...
export type ClinicSettingsClinic = { id: string; name: string; pix_key_type: string | null; pix_key: string | null; pix_holder: string | null; pix_bank: string | null };
export type ClinicSettingsHour = Omit<ClinicHoursInput, "opens_at" | "closes_at" | "break_starts_at" | "break_ends_at"> & { clinic_id: string; opens_at: string | null; closes_at: string | null; break_starts_at: string | null; break_ends_at: string | null };
...
export function ClinicSettingsManager({ clinics, openingHours, canEdit, initialClinicId, embedded = false }: { clinics: ClinicSettingsClinic[]; openingHours: ClinicSettingsHour[]; canEdit: boolean; initialClinicId?: string; embedded?: boolean }) {
  const [clinicId, setClinicId] = useState(initialClinicId ?? clinics[0]?.id ?? "");
  ...
  return <div className={embedded ? "grid gap-4" : "mt-6 grid gap-6"}>
    <Card><CardHeader><CardTitle>Pix e Horarios</CardTitle></CardHeader><CardContent className="grid gap-5">
      ...
      {message ? <p className="text-sm">{message}</p> : null}<Button type="button" disabled={!canEdit || pending} onClick={() => start(async () => { const result = await saveClinicSettings({ clinic_id: clinicId, pix_key_type: pixType, pix_key: pixKey, pix_holder: holder, pix_bank: bank, hours }); setMessage(result.message); })}>{pending ? "Salvando..." : "Salvar configuracoes"}</Button>
    </CardContent></Card>
  </div>;
}
